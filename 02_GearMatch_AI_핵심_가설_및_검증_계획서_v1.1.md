...
> **2026-09-22 개정 (D-09 / Prototype v1.7).** `RUNNERS LIKE YOU` 화면이 P0로 들어왔다.
> "그런 정보를 원하는가"를 묻는 단계는 지났다. 이제 **화면을 놓고 반응을 본다.**

실제 유사 러너 통계 숫자는 **여전히 노출하지 않는다.**
`93% MATCH` 같은 표현을 쓰지 않고, 비교 기준만 문장으로 보여준다.

검증 질문:

> 나와 비슷한 러너가 무엇을 신는지가 장비 선택에 실제로 영향을 주는가?

> 비교 기준(주간 거리 / 목표 거리 / Runner Type / 현재 신발)이 납득되는가?

> Shoes 다음으로 알고 싶은 것이 Activity인가, People인가?
